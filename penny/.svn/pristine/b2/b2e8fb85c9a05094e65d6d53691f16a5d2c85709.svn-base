...
class PFLocaliser(PFLocaliserBase):
    ...
    def __init__(self):
        ...
        self.NUMBER_PREDICTED_READINGS = 50      # Number of readings from the laser scan based on  
        ...
        self.INITIAL_NUM_PARTICLES = 1000
        self.INITIAL_X_POSE_NOISE = 0.07#0.05#1      # Scaling noise in x co-ordinate 
        self.INITIAL_Y_POSE_NOISE = 0.07#0.05#1      # Scaling noise in y co-ordinate
        self.INITIAL_A_POSE_NOISE = 0.1#1            # Scaling the angular rotation noise
        self.INITIAL_GAUSS_SD = 150#1                # Gaussian Standard Deviation - Spread of particles positions
        self.INITIAL_VONMISES_SD = 100#5              # VonMises Standard Deviation - Spread of particles rotations

        # Noise Parameters for Updating Particle Cloud
        self.X_POSE_NOISE = 0.05#0.1              
        self.Y_POSE_NOISE = 0.05#0.1
        self.A_POSE_NOISE = 0.15
        self.GAUSS_SD = 1
        self.VONMISES_SD = 75
       ...
       # Adaptive Monte Carlo Localisation Parameters 
        self.ADAPTIVE = True                    # Switch on/off AMCL
        ...
        self.START_NOISE = 350                  # Limit of num particles above which we will increase the noise proportionally to the number of particles (confidence)
	self.NOT_A_CLUE = 440                   # Same as START_NOISE, increasing the proportional noise even more
        self.K_GAUSS_NOISE = 0.01               # Constants for scaling noise in proportion to no. of particles
        self.K_VONMISES_NOISE = 0.1

    # Roulette Wheel Selection of Index
       ...
       # For AMCL 
        if self.ADAPTIVE: 
            ...
            for i, thisPose in enumerate(poseArray.poses):
                if len(poseArray.poses) > self.START_NOISE:
                    thisPose = self.adding_noise(thisPose, self.K_GAUSS_NOISE*len(poseArray.poses), self.K_VONMISES_NOISE*len(poseArray.poses))
                elif len(poseArray.poses) > self.NOT_A_CLUE:
                    thisPose = self.adding_noise(thisPose, 2*self.K_GAUSS_NOISE*len(poseArray.poses), 2*self.K_VONMISES_NOISE*len(poseArray.poses))                    
                else:
...
'''
        ...
        estpose.position.x = x_sum / self.NUM_PARTICLES
        estpose.position.y = y_sum / self.NUM_PARTICLES
        estpose.orientation.z = qz_sum / self.NUM_PARTICLES
        estpose.orientation.w = qw_sum / self.NUM_PARTICLES
